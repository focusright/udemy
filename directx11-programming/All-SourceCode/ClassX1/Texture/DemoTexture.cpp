...
bool CDemoTexture::LoadContent()
{
    ...
    if (res == false) {
        ::MessageBox(m_hWnd, "Unable to load vertex shader", "ERROR", MB_OK);
        return false;
    }
    ...
    if (res == false) {
        ::MessageBox(m_hWnd, "Unable to load pixel shader", "ERROR", MB_OK);
        return false;
    }
    ...
    hr = CreateDDSTextureFromFile(
        m_pD3DDevice, L"borg2.dds", 0, &m_pColorMap, 0);
    if (FAILED(hr)) {
        ::MessageBox(m_hWnd, "Unable to load texture", "ERROR", MB_OK);
        return false;
    }
    ...
    if (FAILED(hr)) {
        ::MessageBox(m_hWnd, "Unable to create texture sampler state", "ERROR", MB_OK);
        return false;
    }
